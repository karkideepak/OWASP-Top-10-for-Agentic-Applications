> [!note] Attackers manipulate the agent's instructions to divert it from its intended purpose. Unlike standard prompt injection, this often happens "indirectly" via content the agent processes (emails, websites), effectively turning the agent into a "confused deputy" that serves the attacker.
